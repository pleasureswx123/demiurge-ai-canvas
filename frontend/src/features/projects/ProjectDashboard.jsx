...
export default function ProjectDashboard({ onEnterProject }) {
  ...
  const refreshList = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(nodeApi('/project/list'));
      ...
      if (!res.ok) {
        const hint =
          res.status === 404 || String(data.error || '').includes('Not found')
            ? '请先在本机终端启动工程服务：在项目根目录运行 npm run dev（backend/node，监听 3200 端口），并保持窗口不要关闭。'
            : '';
  ...
  const openProject = async (slug) => {
    try {
      const res = await fetch(nodeApi(`/project/load?slug=${encodeURIComponent(slug)}`));
      const data = await res.json();
      ...
      onEnterProject({
        ...
        flow: normalizeFlowAssetUrls(
          data.data?.flow || { nodes: [], edges: [], viewport: { x: 0, y: 0, zoom: 0.85 } }
        ),
      });
    } catch (e) {
  ...
  const renameProject = async (slug) => {
    ...
    try {
      ...
      setRenaming(true);
      const res = await fetch(nodeApi('/project/rename'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const copyProject = async (slug) => {
    try {
      const res = await fetch(nodeApi('/project/copy'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deleteProject = async (slug) => {
    try {
      setDeleting(true);
      const res = await fetch(nodeApi(`/project/delete?slug=${encodeURIComponent(slug)}`), {
        method: 'DELETE',
      });
  ...
  return (
    <div className="h-screen overflow-y-auto bg-[#181818] text-[#eceef2] antialiased">

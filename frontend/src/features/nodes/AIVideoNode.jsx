...
import {
  ...
  getVideoModelConfig,
  videoModelOptions,
} from '../generation/videoGenerationConfig';
import {
  Camera,
  ChevronDown,
  Check,
...
const AIVideoNode = ({ data, selected = false }) => {
  ...
  const pollVideoTaskUntilFinished = useCallback(async (taskId, signal, targetNodeId = nodeId, generationMeta = {}) => {
    ...
    while (true) {
      ...
      const response = await fetch(mediaApi(`/video-task/${encodeURIComponent(taskId)}`), {
        signal,
        headers: {
  ...
  const handleGenerateVideo = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(mediaApi('/generate-video'), {
        method: 'POST',

...
PY_SERVICE_PORT = 8790
# Bump when GPT Image / env handling changes so /api/health shows you restarted the process.
...
def bind_outputs_from_request(handler: BaseHTTPRequestHandler) -> contextvars.Token:
    slug = (handler.headers.get("X-Project-Slug") or "").strip()
    if slug and re.fullmatch(r"[a-zA-Z0-9_-]{1,120}", slug):
        d = (PROJECTS_ROOT / slug / "assets").resolve()
        try:
            d.relative_to(PROJECTS_ROOT.resolve())
        except ValueError:
            return OUTPUTS_CTX.set(OUTPUTS_DIR)
        d.mkdir(parents=True, exist_ok=True)
        return OUTPUTS_CTX.set(d)
    return OUTPUTS_CTX.set(OUTPUTS_DIR)


def video_preview_url(filename: str) -> str:
    od = active_outputs_dir()
    try:
        rel = od.resolve().relative_to(PROJECTS_ROOT.resolve())
        parts = rel.parts
        if len(parts) >= 2 and parts[1] == "assets":
            return f"/api/video-file/{parts[0]}/{filename}"
    except ValueError:
        pass
    return f"/api/video-file/{filename}"
...
def load_dotenv_file(dotenv_path: Path, override: bool = False):
...
def load_env():
    # Load .env first, then let .env.local override local secrets.
    for env_path in ENV_PATHS:
        override = env_path.name == ".env.local"
...
def _local_project_media_path(path_value: str) -> Path | None:
    ...
    if path.startswith("/api/material-library/media/"):
        ...
        candidate = (PROJECT_ROOT / "material-library" / "assets" / name).resolve()
        root = (PROJECT_ROOT / "material-library" / "assets").resolve()
        if str(candidate).startswith(str(root)) and candidate.is_file():
            return candidate
    return None

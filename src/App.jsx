...
import { materializeEphemeralAssetUrls } from './materializeProjectAssets';
import GroupPanelNode from './GroupPanelNode';
import MultiSelectToolbar from './MultiSelectToolbar';
import GroupToolbar from './GroupToolbar';
import MaterialLibraryPanel from './MaterialLibraryPanel';
import SaveToMaterialModal from './SaveToMaterialModal';
import {
  deleteMaterialLibraryItem,
  fetchMaterialLibraryItems,
  saveMaterialLibraryItem,
} from './materialLibraryApi';
import { CanvasUiContext } from './CanvasUiContext';
import HistoryPanel from './HistoryPanel';
import {
  clearConnectionHoverTarget,
  setConnectionHoverTarget,
} from './ConnectionHoverStore';
import { setNodeUiState } from './NodeUiStore';
...
function Flow() {
  ...
  const requestProjectAssetCleanup = useCallback(
    async (paths) => {
      ...
      try {
        const res = await fetch('/api/project/cleanup-assets', {
          method: 'POST',
...
export default function App() {
  ...
  const handleDeleteCurrentProject = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/project/delete?slug=${encodeURIComponent(w.slug)}`, {
        method: 'DELETE',
      });
      const data = await res.json().catch(() => ({}));
      ...
      <ProjectDashboard
        onEnterProject={(payload) =>
          setWorkspace({
            ...
            flow: payload.flow || { nodes: [], edges: [], viewport: { x: 0, y: 0, zoom: 0.85 } },
          })
        }
      />
    );

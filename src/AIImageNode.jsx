...
/**
 * 默认走相对路径 `/api/generate-image`，由 Vite 代理到 Python（与视频节点一致），避免跨域与自定义头 CORS 问题。
 * 若需直连（例如无 Vite），可在 `.env.local` 设置 `VITE_IMAGE_SERVICE_ORIGIN=http://127.0.0.1:8790`。
 */
function getImageGenerateApiUrl() {
  const raw = String(import.meta.env.VITE_IMAGE_SERVICE_ORIGIN || '').trim();
  const origin = raw.replace(/\/$/, '');
  if (origin) return `${origin}/api/generate-image`;
...
const AIImageNode = ({ data, selected = false }) => {
  ...
  const handleSeedanceFaceReview = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('/api/seedance-face-review', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      for (let attempt = 0; attempt < 24 && review.status === 'processing' && review.assetRef; attempt += 1) {
        ...
        const pollResponse = await fetch('/api/seedance-face-review', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            ...(projectSlug ? { 'X-Project-Slug': projectSlug } : {}),
    ...
    try {
      ...
      const persistedSrc =
        projectSlug && result.saved_filename
          ? `/api/project/media/${encodeURIComponent(projectSlug)}/${encodeURIComponent(result.saved_filename)}`

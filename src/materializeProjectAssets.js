...
export async function materializeEphemeralAssetUrls(slug, nodes) {
  ...
  const patchObject = async (obj, kind) => {
    ...
    const putUrl = `/api/project/asset/${encodeURIComponent(slug)}/${encodeURIComponent(fn)}`;
    ...
    const next =
      kind === 'image'
        ? `/api/project/media/${encodeURIComponent(slug)}/${encodeURIComponent(fn)}`
        : `/api/video-file/${encodeURIComponent(slug)}/${encodeURIComponent(fn)}`;
    urlMap.set(src, next);
    obj.src = next;
    obj.name = fn;
    changed = true;

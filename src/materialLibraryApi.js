...
export async function fetchMaterialLibraryItems() {
  const response = await fetch('/api/material-library/list');
...
export async function fetchSeedanceSubjects() {
  const response = await fetch('/api/material-library/subjects');
  const payload = await parseJsonSafe(response);
  if (!response.ok) {
    if (response.status === 404 || String(payload.error || '').trim().toLowerCase() === 'not found') {
      return [];
...
async function uploadLibraryBlob(fileName, blob) {
  const response = await fetch(`/api/material-library/asset/${encodeURIComponent(fileName)}`, {
    method: 'PUT',
    body: blob,
  ...
  if (!response.ok) {
    if (response.status === 404) {
      throw new Error('素材库接口未生效，请重启 node server/deepseek-proxy.js');
    }
...
export async function saveMaterialLibraryItem({ name, category, asset, coverAsset = null }) {
  ...
  const saveResponse = await fetch('/api/material-library/save', {
    method: 'POST',
...
export async function saveSeedanceSubject({
  ...
  const response = await fetch('/api/material-library/subject', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      name,
  ...
  if (!response.ok) {
    if (response.status === 404) {
      throw new Error('主体库接口未生效，请重启 node server/deepseek-proxy.js');
    }
...
export async function updateSeedanceSubject(id, patch) {
  const response = await fetch(`/api/material-library/subject/${encodeURIComponent(String(id || ''))}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function deleteMaterialLibraryItem(id) {
  const response = await fetch(`/api/material-library/item/${encodeURIComponent(String(id || ''))}`, {
    method: 'DELETE',
  });
  const payload = await parseJsonSafe(response);
  if (!response.ok) {
